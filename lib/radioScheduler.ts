...
import { changePlayingStatus, skipSong } from './radio';
        ...
        for (const event of events) {
            ...
            if (eventHandlers[action]) {
                ...
                if (is_recurring) {
                    ...
                    await (conn as Pool).query(
                        `UPDATE radio_events SET executed = TRUE WHERE id = $1`,
                        [id]
                    );
                }
            }
...
export const getEvents = async () => {
    try {
        const result = await (conn as Pool).query(`SELECT * FROM radio_events`, [])
        return result.rows
    } catch (error) {
        return 'err'
    }
  ...
  const formattedDate = timeParts 
    ...
    try {
        const query = `
            ...
            (name, date, action, payload, executed, is_recurring, recurrence_time, recurrence_days)
...
export const updateEventPayloads = async (ids: number[], player: string | number) => {
    try {
      const fetchQuery = `SELECT id, payload FROM radio_events;`;
