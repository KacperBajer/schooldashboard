...
import { deleteEvent } from '@/lib/radioScheduler';
...
const RadioEventsTable = ({data, checked, setChecked, fetchData}: Props) => {
    ...
    <div className='overflow-x-auto'>
            <div className='flex flex-col px-4 '>
                <div className='sticky top-0 flex items-center pb-3 border-b-2 border-border text-gray-300 text-sm px-4'>
                    <Checkbox handleChange={handleSelectAllChange} isChecked={data.length === checked.length} />
                ...
                {data.map((item, index) => (
                    <div className='flex items-center py-2 px-4 border-b-2 border-border' key={item.id}>
                        <Checkbox isChecked={checked.includes(item.id)} handleChange={() => setChecked(prev => prev.includes(item.id) ? prev.filter(eventid => eventid !== item.id) : [...prev, item.id])} />
...
export default RadioEventsTable